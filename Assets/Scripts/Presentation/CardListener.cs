...
namespace CardMatchingGame.Presentation
{
    public class CardsListener : MonoBehaviour, IDataPersistence
    {
        ...
        private void GameOverSecuence()
        {
            ...
            _firstCard = null;
            _secondCard = null;
            PresentationSceneReferenceHolder.MatchingCardsSystem.MatchChecker.CleanUpSecuence();
            _cards.Clear();
        }
                ...
                else
                {
                    ...
                    if (_firstCard.CurrentState == CardView.State.FaceUp && _secondCard.CurrentState == CardView.State.FaceUp && _firstCard != _secondCard)
                    {
                        ...
                        else
                        {
                            ...
                            PresentationSceneReferenceHolder.MatchingCardsSystem.AddMatchedCards(_firstCard, _secondCard);
                        }
                    }
        ...
        private IEnumerator AssignValue(GameData gameData)
        {
            while (startAssigningValues && gameData != null)
            {
                for (int j = 0; j < _cards.Count; j++)
                {
                    ...
                    if (j >= _cards.Count - 1)
                    {
                        PresentationSceneReferenceHolder.MatchingCardsSystem.MatchChecker.WinLevelCont = gameData.winLevelCont;
        ...
        public void SaveData(GameData data)
        {
            ...
            foreach (var card in _cards)
            {
                ...
                if (card != null)
                {
                    ...
                    data.winLevelCont = PresentationSceneReferenceHolder.MatchingCardsSystem.MatchChecker.WinLevelCont;
                }
            }
        }
    }
