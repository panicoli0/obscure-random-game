...
namespace CardMatchingGame.Presentation
{
    public class GridHandlerPresentation : MonoBehaviour
    {
        ...
        internal void SetGrid(int amount)
        {
            ...
            PresentationSceneReferenceHolder.MatchingCardsSystem.MatchChecker.WinLevelCont = 0;
            ...
            if (result == false)
            {
                ...
                var cards = PresentationSceneReferenceHolder.CardsListener.GetComponentsInChildren<CardView>();
                foreach (CardView card in cards) PresentationSceneReferenceHolder.CardsListener.AddCard(card);
            }
        }
